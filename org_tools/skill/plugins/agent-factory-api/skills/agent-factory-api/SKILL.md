...
|                      | `POST /avatars/cards/{card_id}/quality-evaluate` (deterministic structural check)                 |
...
`GET /skills` returns up to `limit` items (default varies, max 100) and includes
a `next_cursor` field in the response. When you need all skills or a skill is
not found in the first page, **you MUST paginate**:

```bash
# Page 1
curl -sS -H "$AUTH" "$BASE/skills?limit=100"
# Response: {"items": [...], "next_cursor": "eyJpZCI6..."}

# Page 2
curl -sS -H "$AUTH" "$BASE/skills?limit=100&cursor=eyJpZCI6..."
# Response: {"items": [...], "next_cursor": null}  # null = last page
```

**When to paginate:**
- Searching for a skill by name (not found on page 1 → continue until `next_cursor` is null)
- Listing all skills or counting total
- Finding bundle components (e.g., skills inside "Skill Hub")

**Do NOT paginate for:**
- `GET /skills/{item_id}` (single item, no pagination)
- Avatar card/role/task list endpoints (these return all results)
...
`POST /avatars/cards/{card_id}/quality-evaluate` judges only structure — orphan Role/Task, empty required fields, stale Skill links — never prose quality. Nothing is stored; it recomputes from the current row every call, so re-calling it right before a write is a cheap way to catch a Card someone else edited since you last checked. Caller must be the Card's owner or a co-manager (`403 forbidden` otherwise).

```bash
curl -sS -X POST -H "$AUTH" -H "Content-Type: application/json" -d '{}' \
  "$BASE/avatars/cards/CARD_UUID/quality-evaluate"
```

```json
{
  "card_id": "uuid",
  "evaluated_at": "2026-09-03T08:12:00Z",
  "status": "READY",
  "summary": {"critical": 0, "high": 0, "medium": 0, "low": 0, "info": 1},
  "findings": [
    {"severity": "INFO", "code": "CARD_NO_MANAGERS", "entity_type": "card",
     "entity_id": "uuid", "path": "card", "field": "manager_emails",
     "message_ko": "...", "message_en": "..."}
  ]
}
```

Optional request body `{"include_advisory": false}` drops LOW/INFO from `findings` only — `summary` always counts every severity regardless. `status` is `NOT_READY` with any CRITICAL finding, `NEEDS_IMPROVEMENT` with CRITICAL-free HIGH/MEDIUM, otherwise `READY`. Branch on `code` (namespaced `CARD_*`/`ROLE_*`/`TASK_*`, e.g. `CARD_NO_ROLES`, `ROLE_NO_TASKS`, `TASK_SKILL_LINK_STALE`, `CARD_RESPONSIBILITY_MISSING`, `TASK_CONTEXT_MISSING`), never on `message_ko`/`message_en` — codes stay stable as rules are added, message text doesn't. `entity_type` is `card`/`role`/`task`/`skill_link`, with `skill_link` entities keyed `{skill_id}:{component_key}`; `path` locates the node, e.g. `card.roles[0].tasks[2]`. `CARD_ROLES_NOT_VISIBLE`/`ROLE_TASKS_NOT_VISIBLE` (INFO) mean some Roles/Tasks exist outside this key's view scope — a clean result alongside either code means nothing *visible* was wrong, not that the Card has no gaps.
