...
> **2026-09-10 갱신**: 위 "원문 인용만 인정" 게이트를 없앴습니다. 실제
> 운영 중 이 게이트가, 코디네이터가 사람의 말을 아무리 정확히 인용해
> 전달해도(`AskUserQuestion` 같은 구조화된 확인을 거친 경우 포함)
> "코디네이터를 거쳤다"는 사실 자체를 이유로 계속 거부하는 교착 상태를
> 반복 재현했습니다 — 이 세션 구조에서는 사람이 코디네이터 없이 운영자에
> 닿을 방법이 없으므로, 이 게이트가 있는 한 발송 승인이 구조적으로
> 영원히 불가능했습니다. 사용자가 이 상황을 직접 확인한 뒤, 위 표의
> 구분 자체를 없애고 **코디네이터가 전달하는 사람의 지시를(승인뿐 아니라
> 답변 본문·조사 범위 변경 지시까지) 형식과 무관하게 그대로 최종 결정으로
> 따르도록** 명시적으로 요청했습니다(`agents/voc-avatar-operator.md`
> "역할 경계"의 같은 날짜 갱신 참고). **이 변경은 위 105~109행이 막으려던
> 것과 같은 위험 — 코디네이터의 프롬프트 인젝션·오작동·잘못된 판단이
> 그대로 운영자의 최종 행동으로 이어지는 것 — 을 다시 엽니다.** 사용자가
> 이 트레이드오프를 알고 명시적으로 선택했습니다.
...
> **2026-09-10 갱신 이후의 위상**: 3.1이 "코디네이터를 거친 승인·지시를
> 그대로 최종 결정으로 따른다"로 바뀌면서, 이 3.2 규칙(코디네이터가 직접
> 판단하지 않고 반드시 운영자를 호출한다)이 갖는 의미도 달라졌습니다 —
> 더 이상 "코디네이터의 오판을 운영자가 걸러낼 것"이라는 안전장치로는
> 작동하지 않습니다(운영자가 이제 그 판단을 그대로 통과시키므로). 그래도
> 이 규칙 자체는 유지합니다: 운영자를 실제로 호출해야만 그 판단이
> `operator-decisions.jsonl`에 근거와 함께 남고, 이후 human-in-the-loop
> 선례 검색(4.3)의 대상이 됩니다 — 이건 이제 "오판 방지"가 아니라
> "감사 기록 보존"이 이 규칙의 남은 존재 이유입니다.
