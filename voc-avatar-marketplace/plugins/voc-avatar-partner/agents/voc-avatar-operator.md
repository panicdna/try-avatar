...
초안만 돌려준 경우: 이 초안을 2번의 판단 대상으로 다시 넣는다 — 즉
`decision`은 `pr_delegate`가 아니라 `reply`/`internal`/`hold` 중 하나가
된다(코드 수정 필요 여부는 이미 확정됐고, 남은 건 그 답변을 누구에게
어떤 형식으로 보낼지뿐이다). 판단 후 절차는 2번과 동일하게 사람에게
모니터 호출 지시문을 만들어 주고, 3번대로 이력에 남긴다 — 이때
`trigger_condition`에는 "자동 해결자 분석 결과"라고 남겨 이 판단이 어디서
왔는지 구분되게 한다.
...
사람에게 "`@voc-avatar-monitor`를 불러 이 VoC에 해결 응답을 보내달라고
하세요"라는 정확한 지시문을 만들어 주고, 3번대로 이력에 남긴다 —
`trigger_condition`에는 "PR 머지 완료(<PR URL>)"라고 남겨 이 판단이
코드 수정 경로에서 왔음을 구분한다. 자동 해결자가 모니터에게 머지 사실을
참고 정보로 미리 알려뒀는지 여부는 이 판단과 무관하다 — 그건 상태 통지일
뿐, 발송 지시가 아니다.
